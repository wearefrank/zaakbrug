...
8. Run the e2e testsuite by using the below Docker-Compose and configuration to validate the changes. You should only need `docker-compose -f ./docker-compose.zaakbrug.dev.yml -f ./docker-compose.openzaak.dev.yml --profile soapui up --build --force-recreate` for this. This command will have the system up and running and then will run the e2e tests automatically.
9. The compatibility of PostgreSQL should also be tested. To do so you should rerun the system with the command `docker-compose -f ./docker-compose.zaakbrug.dev.yml -f ./docker-compose.openzaak.dev.yml -f ./docker-compose.zaakbrug.postgres.yml --profile soapui up --build --force-recreate`. This will also run the tests automatically but the system will use PostgreSQL.
10. Commit you changes on a branch with as message: `build(dependencies): bump f!f version to <new tag>`. Create a PR to have you changes merged to master.
